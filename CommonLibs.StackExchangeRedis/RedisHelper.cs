...
#pragma warning disable CS1591 
    public class RedisHelper : IRedisHelper
    {
        ...
        {
            return _Database.KeyDelete(ConvertT<string, RedisKey>(keys, p => p).ToArray());
        }
        ...
        public List<string> GetAllKeys()
        {
            ...
            if (values != null)
                retValues = ConvertT(values, p => (string)p).ToList();
            return retValues;
        }
        ...
        {
            return new List<string>(ConvertT(_Server.Keys((int)_Db, pattern).ToArray(), p => (string)p));
        }
        ...
        {
            return new List<string>(ConvertT(_Server.Keys((int)_Db, pattern, pageSize, 0, pageOffset), p => (string)p));
        }
        ...
        private IEnumerable<TResult> ConvertT<T, TResult>(IEnumerable<T> sources, Converter<T, TResult> transformation)
        {
